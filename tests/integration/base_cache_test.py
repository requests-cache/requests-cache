...
class BaseCacheTest:
    ...
    def test_cookies(self):
        ...
        response_1 = parse_json(httpbin('cookies/set?test1=test2'))
        with session.cache_disabled():
            assert parse_json(httpbin('cookies')) == response_1

        # From cache
        response_2 = parse_json(httpbin('cookies'))
        assert response_2 == parse_json(httpbin('cookies'))

        # Not from cache
        with session.cache_disabled():
            response_3 = parse_json(httpbin('cookies/set?test3=test4'))
            assert response_3 == parse_json(httpbin('cookies'))
