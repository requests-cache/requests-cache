...
from requests_cache.backends.filesystem import LRUFileDict
...
class TestLRUFileDict(TestFileDict):
    ...
    def test_get_size_on_disk(self, block_bytes, file_size, expected_size):
        ...
        assert cache.get_size_on_disk(file_size) == expected_size
        assert cache.get_size_on_disk(-file_size) == -expected_size
    ...
    def test_block_alignment(self, block_bytes: int):
        ...
        assert block_lfd.size() == block_lfd.get_size_on_disk(100)

        # Replacing items
        block_lfd['file'] = '0' * 123
        assert block_lfd.size() % block_lfd.block_bytes == 0
        assert block_lfd.size() == block_lfd.get_size_on_disk(123)
