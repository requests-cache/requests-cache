...
* {ref}`URL Patterns <url-filtering>`: Since we're using patching, this is a safety measure to avoid
  accidentally caching any non-Github requests elsewhere in your code.
...
* Rate limiting: The GitHub API is
  [rate-limited](https://docs.github.com/en/rest/overview/resources-in-the-rest-api#rate-limiting)
  at 15000 requests per hour if authenticated, or only 60 requests per hour otherwise. This makes
  caching especially useful, because cache hits and `304 Not Modified` responses (from conditional
  requests) are not counted against the rate limit.
* {ref}`inspection`: After calling some PyGithub functions, we can check out some details on
  the actual API requests that were sent.
* {ref}`Security <default-filter-params>`: If you use a personal access token, this will be sent
  in the `Authorization` header. This is likely not something you want to store in an unsecured
  storage backend, so this and other common credentials are redacted by default. This example shows
  how to verify this.
"""
...
def check_cache():
    ...
    print('Example cached request headers:')
